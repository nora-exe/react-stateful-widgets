...
import React, {useState} from 'react'; /* STEP 0: IMPORT STATE */
...
export default function Squares() {
  // STEP 1: states
  const [squares, setSquares] = useState(listOfSquareIds); //holds ARRAY of square IDs
  const [activeSquare, setActiveSquare] = useState(null); //tracks currently active square - none (null) on LOAD
  ...
  const markActive = id => {
    ...
    setActiveSquare((id === activeSquare) ? null : id) //set to null if already active, or set ID active
  };
      ...
      <div className='squares'>
        {
          ...
          squares.map(id => // change to squares, not the array itself
            <div
              id={id}
              key={id}
